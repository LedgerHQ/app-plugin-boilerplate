...
static bool set_send_ui(ethQueryContractUI_t *msg, context_t *context) {
    ...
    switch (context->selectorIndex) {
        ...
        case VAULT_MINT:
            strlcpy(msg->title, "Deposit", msg->titleLength);
            if (memcmp(USDT_ADDRESS, context->contract_address_sent, ADDRESS_LENGTH) == 0 ||
                memcmp(USDC_ADDRESS, context->contract_address_sent, ADDRESS_LENGTH) == 0) {
        ...
        case WRAP:
            ...
            strlcpy(context->ticker_sent,
                    is_woeth ? OETH_TICKER : OUSD_TICKER,
                    sizeof(context->ticker_sent));
            break;
        case UNWRAP:
...
static bool set_receive_ui(ethQueryContractUI_t *msg, context_t *context) {
    ...
    switch (context->selectorIndex) {
        ...
        case VAULT_MINT:
            strlcpy(context->ticker_received,
                    is_oeth ? OETH_TICKER : OUSD_TICKER,
                    sizeof(context->ticker_received));
            break;
        case VAULT_REDEEM:
            strlcpy(context->ticker_received,
                    is_oeth ? "LST MIX" : "USD MIX",
                    sizeof(context->ticker_received));
            break;
        case CURVE_POOL_EXCHANGE:
            break;
        ...
        case UNISWAP_V3_ROUTER_EXACT_INPUT:
        case UNISWAP_ROUTER_EXACT_INPUT_SINGLE:
            if (memcmp(USDT_ADDRESS, context->contract_address_received, ADDRESS_LENGTH) == 0 ||
                memcmp(USDC_ADDRESS, context->contract_address_received, ADDRESS_LENGTH) == 0) {
