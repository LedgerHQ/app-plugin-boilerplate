...
_test.nano_models.forEach(function (model) {
  jest.setTimeout(20000);
  test('[Nano ' + model.letter + '] Swap Exact Eth For Tokens', (0, _test.zemu)(model, async (sim, eth) => {
    const contract = new _ethers.ethers.Contract(contractAddr, abi);

    // Constants used to create the transaction
    // EDIT THIS: Remove what you don't need
    const amountOutMin = (0, _utils.parseUnits)("28471151959593036279", 'wei');
    ...
    const beneficiary = _test.SPECULOS_ADDRESS;
    ...
    const {
      data
    } = await contract.populateTransaction.swapExactETHForTokens(amountOutMin, path, beneficiary, deadline);
    ...
    unsignedTx.value = (0, _utils.parseEther)("0.1");
